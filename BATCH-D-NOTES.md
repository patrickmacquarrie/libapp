...
- US Season 11 is registered as the upcoming public-beta season, but remains unavailable with no checked-in Sheet ID. Publishing/runtime configuration can activate it after the real Sheet is connected.
...
- `npm run test:release-contract` passed with executable checks for the missing-runtime-config failure, the UK3 historical state, the unavailable US11 registration, and activation through published runtime configuration.
...
- Patrick must connect the real US11 source Sheet through Season Admin before cut-over. No Sheet ID was invented or copied from another season.
