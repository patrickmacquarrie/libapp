...
Use **Save draft** for ordinary editing. This updates only the spreadsheet. **Save & preview** validates the sheet, records a hash of that exact release, and produces the normal publisher summary without changing Firestore. **Publish to the app** reads the sheet again and accepts only the exact state from the latest successful preview; it never rewrites the sheet from browser state. The season snapshot, its backup, and matching default-season routing metadata are committed together. Any intervening sheet edit requires another preview. A successful publish consumes the approval, so every publish requires its own preview. **Restore latest backup** uses the reversible rollback flow.

After a Live or Upcoming season has been published, use **Make live/default** on its Overview screen to make it the season the main app opens by default. The same season becomes the active public Global Pool. This is intentionally separate from publishing, so an ordinary content update cannot accidentally move the Global Pool. Previous Global Pool records remain preserved for operational history but are not listed in the player lobby.
