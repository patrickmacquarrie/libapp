...
const scoringEnginePath=path.join(root,'functions','shared','scoring-engine.js');
...
function seasonPage(season,data,availableSeasons) {
  ...
  const castSentence=castNames.length
    ? `Meet ${castNames.slice(0,4).map(esc).join(', ')}${castNames.length>4?`, and ${castNames.length-4} more singles`:''}.`
...
<section class="grid"><article class="card"><h2>Meet the cast</h2><p>${castSentence}</p><ul class="cast">${castList}</ul></article><article class="card how-card"><h2>Make your calls</h2><p>Choose your picks, back them with Hearts, and see how your predictions stack up with friends.</p><div class="checkpoints"><div class="checkpoint"><b>1</b><div><strong>Pick your season</strong><span>Create a private pool and invite your group chat.</span></div></div><div class="checkpoint"><b>2</b><div><strong>Lock your calls</strong><span>Spend Hearts on the predictions you trust most.</span></div></div><div class="checkpoint"><b>3</b><div><strong>Climb the standings</strong><span>Score each checkpoint at your own watch pace.</span></div></div></div></article></section>
<section class="lower-cta"><h2>Turn every episode into a friendly competition</h2><p>Through the Wall scores picks across the pods, retreats, weddings, and reunion.</p><a class="cta" data-placement="season_footer" href="/?start=create&amp;season=${encodeURIComponent(season.id)}">Start ${esc(season.label)}</a></section></main>
<footer><a href="/privacy.html">Privacy</a> · <a href="/terms.html">Terms</a></footer>
<script>document.querySelectorAll('[data-placement]').forEach(link=>link.addEventListener('click',()=>window.ttwAnalytics&&window.ttwAnalytics.track('create_pool_click',{placement:link.dataset.placement,seasonId:'${esc(season.id)}'})));</script></body></html>`;
}

async function build() {
  const source=fs.readFileSync(sourcePath,'utf8');
  const analyticsSource=fs.readFileSync(analyticsPath,'utf8');
  const scoringEngineSource=fs.readFileSync(scoringEnginePath,'utf8');
  ...
  const buildSource=source
    .replace('/* __SCORING_ENGINE_SOURCE__ */',scoringEngineSource)
  ...
  fs.writeFileSync(path.join(dist,'analytics.js'),builtAnalytics);
  ...
  const snapshotFactory=scoringEngineSource.slice(scoringEngineSource.indexOf('const rulesSnapshotFrom'),scoringEngineSource.indexOf('const configForPool'));
