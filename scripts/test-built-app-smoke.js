...
async function main() {
  ...
  const workerHeaders=hosting.slice(workerHeaderStart,workerHeaderEnd);
  assert(workerHeaderStart>=0&&workerHeaders.includes('"Cache-Control", "value": "no-cache, no-store, must-revalidate"'),'Firebase Hosting must never cache the service-worker kill switch.');
