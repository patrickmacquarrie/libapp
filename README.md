...
The app and Cloud Functions both use the published Firestore `seasons/{seasonId}` snapshot. A season is loadable once that snapshot contains Settings. Cast, Couples, Dating Results, and Reunion Results may be empty before their data is known, which allows live pre-drop pools to form at Episode 0. Predictions remain closed until a viable cast is released and the first Pods episode is available. The browser does not silently fall back when Firestore or Settings is missing.
