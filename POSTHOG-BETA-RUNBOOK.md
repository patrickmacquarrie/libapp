...
7. Keep these five saved insights with the internal/test-user filter enabled:
   ...
   - **Paid acquisition by source:** daily trends for `account_created`, `pool_created`, `global_pool_joined`, and `first_checkpoint_locked`, filtered to `acquisition_source` beginning with `paid_` and broken down by `acquisition_source`.
   - **Activation within 7 days:** `account_created` → `first_checkpoint_locked`, sequential with a seven-day conversion window and broken down by `acquisition_source`.
   - **Pricing research:** `price_prompt_shown` and `price_response`, broken down by displayed `price` and `response`.
8. Create retention views from `sign_in_completed`, `episode_return`, `first_checkpoint_locked`, and `return_visit`. `first_checkpoint_locked` fires on the first phase a player locks in each pool, including players who join after Pods. For pricing analysis, use `price_prompt_shown` as the denominator, then break `price_response` down by the displayed `price`, `response`, and `member_count`; keep `price_variant` as a diagnostic breakdown. Count only each identified person’s earliest `price_response`, including `dismissed`; browser storage suppresses ordinary repeats on one device but is not the deduplication source of truth across devices. Calculate the yes rate as `yes / (yes + maybe + no)`, excluding `dismissed`, and report dismissals separately as a share of `price_prompt_shown`.
