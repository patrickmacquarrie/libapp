...
6. The beta lifecycle event list includes `app_arrival`, `sign_in_completed`, `account_created`, `pool_created`, `invite_sent`, `invite_link_opened`, `invite_accepted`, `episode_return`, `first_checkpoint_locked`, `return_visit`, `notif_opt_in`, `price_fakedoor_click`, and `founding_email_captured`.
...
8. Create retention views from `sign_in_completed`, `episode_return`, `first_checkpoint_locked`, and `return_visit`. `first_checkpoint_locked` fires on the first phase a player locks in each pool, including players who join after Pods. Create a pricing view broken down by `price_variant` for `price_fakedoor_click` and `founding_email_captured`.
