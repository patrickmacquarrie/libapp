...
6. The beta lifecycle event list includes `app_arrival`, `sign_in_completed`, `account_created`, `pool_created`, `invite_sent`, `invite_link_opened`, `invite_accepted`, `episode_return`, `first_checkpoint_locked`, `return_visit`, `notif_opt_in`, `price_prompt_shown`, and `price_response`.
